...
/*   Updated: 2026/08/23 14:41:52 by benpicar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static uint32_t			g_heap_break = 0;
static uint32_t			g_heap_mapped_end = 0;
static t_block_header	*g_heap_head = NULL;
static t_block_header	*g_heap_tail = NULL;
...
uint32_t	kbrk(int32_t increment)
{
	...
	if (g_heap_break == 0)
	{
		g_heap_break = KHEAP_START;
		g_heap_mapped_end = KHEAP_START;
	}
	old_break = g_heap_break;
	new_break = old_break + increment;
	if (increment < 0 && new_break < KHEAP_START)
		return (KBRK_FAILED);
	while (new_break > g_heap_mapped_end)
	{
		frame = frame_alloc();
		if (frame == FRAME_ALLOC_FAILED)
			return (KBRK_FAILED);
		if (paging_map_page(g_heap_mapped_end, frame, PAGE_RW) != 0)
		{
			frame_free(frame);
			return (KBRK_FAILED);
		}
